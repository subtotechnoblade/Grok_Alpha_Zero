...
class Batch_Dense(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        return tf.vectorized_map(self.dense, inputs)
...
class Batch_Conv1D(tf.keras.layers.Layer):
    ...
    @tf.function
    def call(self, inputs):
        return tf.vectorized_map(self.conv1D, inputs)
...
class Batch_Conv2D(tf.keras.layers.Layer):
    ...
    @tf.function
    def call(self, inputs):
        return tf.vectorized_map(self.conv2D, inputs)
...
class Batch_Conv3D(tf.keras.layers.Layer):
    ...
    @tf.function
    def call(self, inputs):
        return tf.vectorized_map(self.conv3D, inputs)
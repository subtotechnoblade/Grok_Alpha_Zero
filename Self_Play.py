...
class Self_Play:
    def __init__(self,
        ...
        if not os.path.exists(f"{folder_path}/{generation}/Self_Play_Data.h5"):
            raise ValueError("Dataset file hasn't been created")
        ...
        self.sess = rt.InferenceSession(f"{self.folder_path}/{self.generation}/TRT_cache/model_ctx.onnx",
                                        providers=providers)
    ...
    def play(self):
        ...
        with self.lock, h5.File(f"{self.folder_path}/{self.generation}/Self_Play_Data.h5", "r+") as file:
            ...
            dataset_name = f"{(len(file.keys()) - 1) // 3}"
...
def self_play_task(game_id, game_class,
                 build_config:dict,
                 train_config:dict,
                 lock: mp.Lock,
                 folder_path:str,
